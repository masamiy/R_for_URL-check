...
Tailord by @joelnitta (https://github.com/joelnitta/url-check-chunks)

This takes a CSV file as input and checks the status of URLs in chunks in parallel.

1. Unzip a file so the folder strcture required for the prihect is ready.
There are 5 folders:  
...
2. Open a project, url-check-chunks.Rproj
3. Adjust name of the input data file and number of chunks as needed. Values needed to be adjusted are:
...
4. Run the code in `R/check_url_in_chunks.R`, after adjusting name of the input data file and number of chunks as needed. Values needed to be adjusted are:  
